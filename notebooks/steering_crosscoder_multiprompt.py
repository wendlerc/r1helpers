...
def gen(lm, toks, n_new_toks=50, temperature=0.6, do_sample=True, top_p=0.95, seed=42):
    ...
    input_ids = torch.tensor([toks], dtype=torch.long).to(
        lm.device)  # Move to model's device
...
def gen_batch(lm, toks_batch, n_new_toks=50, temperature=0.6, do_sample=True, top_p=0.95, seed=None):
    ...
    for i, t in enumerate(toks_batch):
        input_ids[i, -len(t):] = torch.tensor(t, dtype=torch.long,
                                              device=lm.device)  # Move to model's device
...
def steer_batch(lm, toks_batch, vecs,
    ...
    B = len(toks_batch)
    # Create on model's device
    fire_cnt = torch.zeros(B, dtype=torch.int64, device=lm.device)
...
def make_batches(dataset, batch_size):
    items = list(dataset)
    # length-bucketing from longest to shortest
    items.sort(key=lambda d: len(d["subsequence_tokens"]), reverse=True)
